...
I tried both local and Azure workbook svc too but faced the same error. >> "Module could not be found". I tried re-installing reqd lib's and restarted the kernel - no improvement. 
...
To resolve this, you may run "conda activate <environment-name>" before installing the package. 
...
Alternatively, you may also set the PATH environment variable so that it points to the environment of the target conda environment as follows, this may be done from the Notebook. 
	...
	2. Update the envs path and set it to $PATH environment variable. For example,
...
!pip list 
```
Also, set the right SDK environment to execute the notebook. If you change the runtime, then reselect the machine by switching between the compute instances say VM -> Serverless and then to VM and then select the right version to see them working fine. 
...
file1.close()
```
Good reference for python commands - https://www.geeksforgeeks.org/writing-to-file-in-python/?ref=lbp
